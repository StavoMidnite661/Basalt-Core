  ...
  return (
    <div className="w-full bg-basalt-bg border-b border-basalt-800 font-mono z-10 shrink-0 select-none shadow-sm shadow-black/50">
      <nav 
        ...
        className="flex items-center px-6 py-4 gap-4 lg:gap-6 w-full overflow-x-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] scroll-smooth"
      >
        {workflowStages.map((stage, stageIdx) => (
          <React.Fragment key={stage.group}>
            <div className="flex flex-col flex-shrink-0">
              <span className="text-[10px] text-basalt-orange font-black tracking-widest uppercase mb-2 opacity-90 pl-0.5">
                {stage.group}
              </span>
                ...
                {stage.items.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex items-center justify-center gap-2.5 h-10 px-4 transition-all border outline-none whitespace-nowrap cursor-pointer ${
                        isActive 
                      ...
                      title={item.label}
                    >
                      <Icon className={`w-4 h-4 ${isActive ? 'text-authority-cyan' : 'text-zinc-500 group-hover:text-zinc-400'}`} />
                      <span className="text-[10px] lg:text-[11px] font-bold tracking-widest uppercase">
                        {item.label}
                      </span>
                    </button>
                  );
